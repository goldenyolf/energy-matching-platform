# 綠電媒合平台（Energy Matching Platform）

一個以台灣企業綠電轉供（Corporate PPA）為題的作品集專案。它把「風場實際發了多少電、
企業實際用了多少電、手上有哪些合約」這三件事拆開來各自計算，再用一套可重現、可稽核的
媒合引擎，算出每家企業每個月真正拿到多少綠電、離 RE 目標還差多遠——最後把整套決策流程
（媒合、售電評估、投資效益、轉供結算、憑證追蹤、風險告警）收成一個售電業會用得上的介面。

### 🔗 快速連結

| | |
|---|---|
| 📖 **[產品介紹 · 演算法圖解](https://goldenyolf.github.io/energy-matching-platform/)** | 小白也能懂的視覺化解說（GitHub Pages，隨開隨看） |
| 🗺️ **[產品 Roadmap 看板](https://goldenyolf.github.io/energy-matching-platform/roadmap.html)** | 領域×階段矩陣、45 張功能卡、依賴連線（`docs/PRD.md` 視覺化） |
| 🧲 **[任務看板（拖拉）](https://goldenyolf.github.io/energy-matching-platform/board.html)** | 待辦／進行中／測試中／完成，可拖拉卡片（狀態存瀏覽器 localStorage） |
| 🚀 **[線上 Demo](https://emp-api-bxbe.onrender.com/app/)** | 完整平台（Render 免費方案，首次開啟可能要等冷啟動） |
| 📑 API 文件 | 上方 Demo 網域的 `/docs`（Swagger UI） |

<!-- 系統架構與資安入口暫時隱藏（對外分享期間）。要恢復就把這一列搬回上面的表格：
| 🏗️ **[系統架構與資安](https://goldenyolf.github.io/energy-matching-platform/architecture.html)** | 現況 vs 企業正式部署（GCP·資料落地台灣）的 icon 關係圖、分層資安、遷移路徑 |
-->

> **聲明**：站上所有數字都是**模擬示範資料**，與台電、TSEC 或任何能源業者無關；
> 只有「即時再生能源」頁抓的是台電公開的即時資料。這是技術作品集，不是正式的結算、
> 憑證移轉或交易系統。

---

## 這個專案想解決什麼

合約上寫「佔某風場 70%」，不等於這家公司這個月真的拿到 70% 的綠電。實際拿到多少，取決於
風場當月發了多少、同一座風場還有沒有其他優先序更高的合約在搶、以及企業自己用了多少電。

多數試算把「合約比例」直接當成「已供綠電」，結果失真。這裡的做法是把發電、用電、合約、
分配當成四個獨立的量，照合約優先序逐一分配，並且**每一筆分配都記下它是被哪個上限卡住的**
（風場剩餘、客戶剩餘需求，還是合約上限）。同樣的輸入，永遠得到同樣的結果。

## 功能

平台的頁面大致分三塊：

**資料管理** — 發電案場（離岸／陸域風電與**太陽能**）、企業客戶、綠電合約的基本資料；
用電還能再拆到**電號／廠區**層級（對應多廠區企業，各廠區有各自的 RE 目標）。

**媒合與評估** — 這是核心：

- 依合約優先序的月結媒合（稽核基準，貪婪、可解釋）。
- 全域經濟最佳化（MILP）：在 RE 目標與結構限制下，讓售電業毛利最大。
- 逐時段（台電三段式時間電價）媒合，含二次匹配、最小化時段錯配。
- 售電評估：對選定客戶跑一次最佳化，同時產出賣方毛利與買方 RE 達成／成本。
- 逐時（24/7 CFE）匹配，含**風光互補**與**儲能時間位移**：太陽能正午 bell 補風電白天缺口、
  電池把外溢挪到缺口時段，並直接給出「只風電 vs 風光 vs 風光＋儲」的 CFE 增益
  （系統級與逐客戶）。
- 投資效益（各案場與組合的 CAPEX、ROI、回收期）。
- RE 目標建議：對未達標客戶，以成本最低優先，建議簽哪些有剩餘綠電的風場補足缺口。
- 多對多匹配（情境模擬）：自選要納入的案場／用電端、自訂各家 RE 目標，以全域最佳化算出跨越既有合約的「what-if」綠電配置組合。

**監控與結算** — 轉供結算單（逐時段電量金額、輸配費、售電毛利、減碳量）、
合約風險告警（到期／供電不足／超額承諾／狀態不一致，分三級）、
**合約詳情**（單一 PPA 一整年的逐月履約、每個月被哪個約束卡住、買賣雙方與售電業毛利的雙面帳）、
T-REC 憑證帳（發行、移轉、註銷的生命週期），以及抓台電即時發電的再生能源監控。

底層是一套純函式、無 I/O 的媒合引擎，外面包 FastAPI + SQLAlchemy 2 的分層架構，
前端是零相依的靜態 SPA，附 Alembic 遷移、Docker、GitHub Actions CI 與完整測試。

## 畫面

前端是一支零相依、免 build 的單頁式介面，由 API 同源服務在 `/app/`（示範資料）。

**總覽** — 一個期間的整體發電、分配、RE 達成與風場利用率。右上角琥珀色的「示範資料」
標記代表模擬資料；到「即時再生能源」頁會變成綠色「即時 · 台電資料」——那是唯一的真實來源。

![總覽儀表板](docs/images/dashboard-overview.png)

**資料管理（新增／編輯／刪除）** — 發電案場、企業客戶、電號／廠區與綠電合約都能直接在平台上
維護。以綠電合約為例：一鍵新增（動態挑選發電案場與用電端、設定期間、合約電量或比例、轉供價格、
優先序與狀態），每列可編輯或刪除；合約雙方屬「僅新增」（改對象＝簽新約），刪除若仍被媒合結果
引用會被擋下。寫入可用 `ADMIN_WRITE_TOKEN` 加上輕量密碼保護（未設定則開放）。

![綠電合約 CRUD](docs/images/contract-crud.png)

**CSV 整批匯入（範本、預覽、逐列錯誤）** — 案場、客戶、合約、電號、電池、發電、用電七種實體都能整批
匯入。面板一打開就看得到「下載範本 CSV」與可折疊收合的欄位說明（避免十幾列欄位文件把選檔按鈕擠出
可視範圍）；選檔案後自動跑一次 dry-run 預覽——新增／更新／略過／錯誤四個計數，逐列列出關鍵值與動作，
錯誤依（欄位、原因）分組，不會被兩千列同一種錯誤洗版。按下「確認匯入」才真的寫入，依自然鍵
（如案場代碼、合約編號）比對，已存在者更新、內容相同則略過——同一份檔案重複匯入不會造成資料重複。

![CSV 匯入預覽](docs/images/csv-import-preview.png)

**售電評估(旗艦頁)** — 選一位客戶,一次 MILP 求解同時導出彼此一致的雙面觀點:
賣方毛利與買方 RE 達成／成本,加上逐風場分配(附「被哪個上限卡住」的原因)與逐時段
別達成。進頁即自動載入。

![售電評估](docs/images/sales-evaluation.png)

評估裡的**時段別供需匹配圖**把「風電進來怎麼對上這家客戶的用電」一眼講清楚——每個台電
時間電價時段一根長條(高＝用電、綠＝已分配綠電、灰＝缺口):風電離峰(夜間)多,離峰達 100%;
尖峰(日間)用電最高、風電最弱,只有 35%,還有一批綠電因時段錯配而外溢。這正是「合約比例≠
實際綠電」要靠逐時段最佳化才看得準的地方。

![時段別供需匹配](docs/images/slot-match.png)

**逐時（24/7 CFE）匹配** — 把「時段別」再細化到 **24 小時**，正式化為國際的 **24/7 無碳能源
（CFE）指標**。只有「同一小時內發電與用電**重疊**」的部分才算真綠電：發電時沒人用（外溢）不算、
用電時沒風（缺口）也不算，且**嚴格不跨區間**。圖中綠色＝已匹配、灰色＝缺口、藍線＝發電
（超出用電輪廓即外溢）。系統**帳面 RE% 63.4% → 逐時 CFE% 60.9%**（此為含風光與儲能後的最終值），差距即時間錯配；
逐客戶更明顯——白天班的面板廠 83.8%→68.8%，而 24 小時平載的半導體無落差。可切換「全系統／
單一客戶」檢視。下方**時×日 CFE 熱力圖**（31 天 × 24 小時，綠色越深 CFE 越高）一眼看出夜間偏綠
（風強）、白天偏淡（風弱、需求高），並有逐日風況變異。引擎吃**逐日 15 分鐘 interval 資料**
（`interval_readings` 表；示範為模擬多日、校準到月總量），真實 AMI 以同一管線匯入即無縫替換。

**風光互補** — 風電夜強日弱，白天正是工業用電最兇的時候；太陽能的正午 bell 剛好補這個洞。
太陽能不是新資料表，而是同一張案場表裡 `farm_type="solar"` 的發電資產，整條合約／發電／
匹配管線沿用，**媒合引擎零改動**——互補是自然浮現的，不是特別寫出來的。頂部讀數把同一批用電
拿掉光電案場與其合約再算一次當對照：全系統 **只風電 58.8% → 風光 60.2%（+1.4 pt）**；系統級會被
沒簽光電的大客戶稀釋，真正簽了光電的那家（電源管理）是 **73.3% → 89.4%（+16.1 pt）**，
在客戶表的「風光增益」欄逐家列出。24 小時圖上，虛線＝只風電、實線＝風光合計，中間那條琥珀色帶
就是太陽能填進去的白天缺口。

**儲能時間位移** — 逐時匹配有一條鐵律：**嚴格不跨小時**——發電時沒人用是外溢、用電時沒電是缺口，
兩邊互不相抵。`match_hourly` 這支引擎**一行都沒被改動**，鐵律原封不動；儲能是唯一能合法打破
這條鐵律的東西——把外溢的綠電存起來，挪到缺口時段放出。示範資料裡，用電廠 2（電源管理）掛了
一具 **120 MWh／30 MW／88% 往返效率**的電池：頂部讀數因此擴成三段式**只風電 58.8% → 風光 60.2%
→ 風光＋儲 60.9%（+0.7 pt）**——儲能的系統級貢獻只有 +0.7 pt；真正裝了電池的用電廠 2 自己則從
**89.4% → 96.8%（+7.4 pt）**，系統數字被沒裝電池、卻佔投組約 74% 用電的用電廠 1 稀釋掉了。
KPI 列也把電池的能量帳攤開：**儲能送出 3,691 MWh（充入 4,314、損耗與期末殘留 623）**——發電只會
落在三個互斥的桶（直供客戶／充進電池／外溢），所以「外溢」那格是「沒人用，**也沒存進電池**」，
不會把停在電池裡、其實沒人用到的電偷偷算成戰功。電池的充電來源可以來自任何一座案場的外溢
（跨合約），但自家合約優先；跨合約充電目前屬於**情境模擬**，轉供結算單與 T-REC 尚未反映充放。

![逐時（24/7 CFE）匹配 · 含風光互補與儲能時間位移增益](docs/images/cfe-hourly.png)

儲能的效果在**逐客戶檢視**才看得清楚——切到真正裝了電池的用電廠 2：綠色已匹配區上方的**斜線帶**
就是電池放出來的電（原本會外溢，被挪到早晚缺口），下方 **SOC 曲線**（日均水位）看得出「夜間充滿、
白天放掉、傍晚再補」的節奏。在全系統圖上同一批電量只佔圖高約 1%，因為它要跟整個投組的發電量
比例尺；到了這家客戶自己的圖上，它佔該客戶用電的 7.4%，就變成一眼可見的一層。

![逐客戶檢視：儲能放電斜線帶與 SOC 曲線](docs/images/cfe-storage-customer.png)

**多電號／廠區** — 把一家公司的用電拆到各電號／廠區，每個廠區有自己的 RE 目標。
客戶的綠電依目標優先分配（目標高的先填），所以各廠區的 RE% 會不同——圖中用電廠 1的
台南廠（目標 90%）達標，高雄廠則未達。核心媒合仍維持客戶層級。

![多電號 RE 達成](docs/images/multi-meter.png)

**投資效益** — 各風場與整體組合的 CAPEX、年淨利、投報率與靜態回收期；每 MW 建置成本
與 O&M 費率可自行覆寫。

![投資效益](docs/images/investment.png)

**RE 目標建議** — 對未達 RE 目標的客戶,以成本最低優先,建議簽哪些還有剩餘綠電的風場
來補足缺口。每筆列出可補電量、佔缺口比例、躉售價與估計成本,並標記是「擴約」或「新簽」,
以及能否完全補足(圖中用電廠 1缺口大、風場剩餘有限,故僅能補一部分)。

![RE 目標建議](docs/images/re-recommend.png)

**多對多匹配（情境模擬）** — 一個「what-if」情境模擬器：自選要納入的發電案場與用電端、
自訂各家 RE 目標，再重算「哪個案場配多少給誰」的最佳綠電組合，畫成 Sankey 式流量圖
（帶寬正比於配置電量、實線＝既有合約、虛線＝假設新配對）。任一被選案場都能假設性地供電給任一被選客戶。
底層是一次全域最佳化（MILP），三階段字典序求解：先讓 RE 缺口最小、再讓售電毛利最大、
最後把綠電按目標比例公平均攤；每家達成率不會超過設定的目標，多的綠電留為案場餘電。

![多對多匹配](docs/images/match-map.png)

> 🎬 **演算法圖解**：想看它背後的最佳化邏輯（MILP、三階段字典序、公平均攤），有一份小白也能懂的
> [視覺化解說頁](https://goldenyolf.github.io/energy-matching-platform/)（GitHub Pages），
> 原始檔在 [`docs/index.html`](docs/index.html)。

**轉供結算單** — 針對某客戶、某期間產出的正式雙方結算單，由同一套逐時段引擎導出：
最上方同樣有**時段別供需匹配圖**（風電怎麼對上這家的用電），接著是逐時段的綠／灰電量
與金額、客戶應付（綠電轉供費＋台電輸配費）、風場應收、售電業毛利，以及減碳量（tCO₂e）。
轉供價與輸配費可覆寫。

![轉供結算單](docs/images/settlement-bill.png)

**合約風險告警** — 掃描所有合約的四類風險（即將到期、供電不足、風場超額承諾、狀態不一致），
依嚴重度（高／中／低）排序，每筆列出影響對象、說明與建議動作。供電不足沿用媒合引擎計算。

![合約風險告警](docs/images/risk-alerts.png)

**合約詳情** — 合約清單的每一列都點得進去。清單只講「紙上寫什麼」，這一頁講**這紙合約實際跑成什麼樣**：
選定年度的十二個月，每個月拿到多少、被哪個約束卡住、有沒有低於保證量、以及三方各自的帳。

最上面那條十二格的分佈條是整頁的重點——**綁定約束本身就是商務結論**，三種讀法各對應一個決策：

- **被合約上限卡住**＝客戶要得比合約允許的多。若案場當月還有餘電、客戶也還有沒被綠電覆蓋的用電，
  那就是**有加購空間**（三個條件缺一，這句話就不會出現——沒有餘電時調高上限也拿不到電）。
- **被案場供給卡住**＝案場的電被分光了。示範資料的 PPA-2024-004 全年十二個月都是這一種，
  只拿到上限的 67%，因為它排在優先序 3，同案場另有一紙優先序更高的合約先分。
  這時該談的是優先序或案場是否超賣，不是調高上限。
- **被客戶用電卡住**＝合約簽得比客戶用得掉的多，多簽的部分是浪費；若合約帶 take-or-pay 還會賠錢。

![合約詳情：全年被什麼卡住](docs/images/contract-detail.png)

月別履約圖裡，**柱是實際分配、短橫是該月上限、虛線短橫是 take-or-pay 門檻**；上限畫成每月一段短橫
而不是一條連續折線，因為沒設上限的月份根本沒有值，連起來等於畫出一條不存在的限制。
點任一月展開當月明細，右上角直接附上**引擎產生的原始分配理由**（`allocated 1250.0 MWh (limited by contract cap)`），
數字可以回頭稽核，不必信任畫面。

**雙面帳**把買方應付、案場應收、售電業毛利擺在同一頁分欄標示。這裡不需要任何分攤假設——
所有費率都是 per-kWh，take-or-pay 門檻本來就是合約層級的，所以「這紙合約的帳」算得出來，
不是從客戶層級的總數硬拆下來的。

![合約詳情：單月明細與雙面帳](docs/images/contract-detail-bill.png)

> ⚠️ **本頁是履約基準，不是最佳化基準。** 分配來自依合約優先序的月結引擎（「依約該拿到什麼」），
> 轉供結算單頁則採 MILP 最佳配置，**兩頁的金額會有落差**。頁面底部固定寫明這件事，也寫明輪供費率、
> 以及案場沒設躉售價時成本側是用預設值試算——給售電業看毛利卻不講成本哪來的，遲早出事。
> 同樣的原則貫穿整頁：未設上限顯示「未設上限」而不是 0%，未生效的月份顯示「–」而不是 0，
> 沒設售電價的合約整個金額區塊不顯示，而不是印一排 0。

**T-REC 憑證** — 持久化的憑證帳，兩段式生命週期：媒合即「發行＋移轉」給客戶
（1 憑證 = 1 MWh），客戶再「註銷」抵充 RE。可互動——「發行本期憑證」由當期媒合結果產生，
每筆已移轉的憑證都能按下「註銷」。

![T-REC 憑證帳](docs/images/trec-ledger.png)

**即時再生能源** — 唯一的真實資料：台電公開的各機組即時發電（dataset 8931，約 10 分更新），
read-through 呈現、不進媒合引擎。綠色「即時 · 台電資料」標記與其他頁的琥珀色「示範資料」對照。

![即時再生能源（台電真實資料）](docs/images/live-renewables.png)

**手機 / RWD** — 介面完整響應式：側邊欄收合成置頂的橫向導覽列、KPI 降成兩欄、
寬表格在卡片內橫向捲動（頁面本身不會左右晃）。

<p>
  <img src="docs/images/mobile-overview.png" alt="手機版總覽" width="300">
  &nbsp;
  <img src="docs/images/mobile-investment.png" alt="手機版投資效益" width="300">
</p>

## 技術與架構

FastAPI · SQLAlchemy 2 · Pydantic 2 · Alembic · PostgreSQL；媒合最佳化用 PuLP + CBC；
前端純 HTML/CSS/vanilla JS；Ruff / Black / mypy / pytest；Docker、GitHub Actions CI。

```
app/
├── api/v1/        # FastAPI 路由（風場、客戶、合約、發電、用電、媒合、分析、憑證）
├── schemas/       # Pydantic v2 請求／回應模型
├── services/      # 商業邏輯與流程編排
├── matching/      # 純粹、可重現的媒合引擎（無 I/O）
├── ingestion/     # CSV 匯入、DataSource 介面、mock 產生器
├── repositories/  # ORM 上的泛型 CRUD
├── models/        # SQLAlchemy 2.x 實體
├── core/          # 設定、領域例外
└── db/            # engine、session、declarative base
web/               # 靜態 SPA（index.html + styles.css + api.js + app.js）
alembic/           # 資料庫遷移
data/sample/       # 示範 CSV
```

更完整的架構圖、ERD 與媒合流程，見
[`docs/architecture.md`](docs/architecture.md)、
[`docs/domain-model.md`](docs/domain-model.md)、
[`docs/matching-rules.md`](docs/matching-rules.md)。

## 媒合怎麼算的

以一個月為單位，先加總各風場發電量與各客戶用電量。接著把「有效且在合約期間內」的合約
依 `priority` 排序（平手時看 `start_date`、再看 `contract_number`），逐一分配
`min(風場剩餘, 客戶剩餘需求, 合約上限)`——合約上限取「固定電量」與「佔風場發電的比例」
兩者中較緊的那個。同一座風場的電不會被分配兩次，客戶也不會超過自己的用電。

**三者中真正咬到的那一個會被記下來**，這就是合約詳情頁「被什麼卡住」的來源：引擎每筆分配都寫下
綁定約束（案場供給／客戶用電／合約上限），同時綁定兩個以上時取較硬的那個（供給 &gt; 用電 &gt; 上限
——案場沒電了，調高上限也拿不到）。細節見 [`docs/matching-rules.md`](docs/matching-rules.md)。

## 本機執行（免 Docker）

需要 **Python 3.12**，建議用 [`uv`](https://docs.astral.sh/uv/)。

```bash
make install        # 安裝相依（優先用 uv，沒有就退回 venv + pip）
make seed           # 載入示範資料（預設 SQLite，會自動展開時段、電號、憑證）
make run            # 啟動 API + SPA → http://localhost:8000/app/
```

跑一次媒合、看分析結果：

```bash
curl -X POST http://localhost:8000/api/v1/matching/runs \
     -H 'Content-Type: application/json' -d '{"period":"2024-01"}'
curl 'http://localhost:8000/api/v1/analytics/customers?period=2024-01'
```

`2024-01` 的示範情境（`make seed` 後跑媒合）大致長這樣：

| 客戶 | 用電 | 已分配 | RE % | 目標 | 達標 |
|------|------:|------:|-----:|-----:|:---:|
| 用電廠 1 | 416,667 | 232,759 | 55.9 % | 100 % | ✗ |
| 用電廠 2 | 50,000 | 46,552 | 93.1 % | 100 % | ✗ |
| 用電廠 3 | 83,333 | 55,862 | 67.0 % | 60 % | ✓ |
| 用電廠 4 | 10,000 | 10,000 | 100 % | 80 % | ✓ |
| 用電廠 5 | 4,000 | 4,000 | 100 % | 50 % | ✓ |

已到期與待生效的合約會被跳過（同時也會被風險告警抓出來）。

## Docker 與雲端部署

```bash
docker compose up --build     # 起 PostgreSQL + API（同時服務 SPA）
make docker-seed              # 把示範資料載進容器的 DB
```

- SPA → <http://localhost:8000/app/>，Swagger → <http://localhost:8000/docs>
- API 容器啟動時會先跑 `alembic upgrade head`

雲端有兩條路，都用同一份 `Dockerfile` 搭配 **Neon** Postgres：

- **Render**：一鍵 [`render.yaml`](render.yaml) 藍圖，見 [`docs/deployment.md`](docs/deployment.md)。
  （線上 demo 就跑在這；根目錄 `/` 會自動導向 `/app/`。）
- **Google Cloud Run**：scale-to-zero、`asia-east1`，一行部署腳本
  [`scripts/deploy_cloudrun.sh`](scripts/deploy_cloudrun.sh)，見
  [`docs/deployment-cloudrun.md`](docs/deployment-cloudrun.md)。

## 真實資料：台電風電開放資料

除了內建的示範資料，seeder 也能載入台電每月風力發電的**真實**開放資料
（[data.gov.tw #29961](https://data.gov.tw/dataset/29961)，政府資料開放授權）。
它把逐機組的資料彙整成「一站一風場（代碼前綴 `TPC-`）、一個月一筆發電量」，
取資料中最近 N 個月（預設 12，可能跨年）。這些 `TPC-` 風場會和 `WF-` 示範風場並存。

```bash
python -m scripts.seed --reset --source taipower --fetch    # 線上抓 CSV，載最近 12 個月
python -m scripts.seed --source taipower --months 24        # 用本機 CSV 離線載入
```

台電只公開供給側資料，所以這個來源的客戶／合約／用電會是空的；資料也只涵蓋台電自有
（多為陸域）機組，離岸 IPP（海能、彰芳等）不在其中。

## 測試與開發

```bash
make test       # pytest + 媒合核心覆蓋率
make lint       # ruff + black --check + mypy
make format     # black + ruff --fix
make migrate    # alembic upgrade head
pre-commit install
```

CI（GitHub Actions）跑 ruff、black、mypy 與 pytest；媒合核心覆蓋率要求 ≥ 80%。

## 資料來源與聲明

站上資料全為示範用途、皆屬模擬。日後若要串接任何公開開放資料，都必須遵守來源網站的
使用條款與存取規範——本專案不繞過任何驗證、robots.txt 或流量限制。即時再生能源頁使用的
台電公開資料即依此原則 read-through 取用、不另存、不再散佈。

## 授權

[MIT](LICENSE)
