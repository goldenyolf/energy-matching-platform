...
- 逐時（24/7 CFE）匹配，含**風光互補**：太陽能正午 bell 補風電白天缺口，並直接給出
  「只風電 vs 風光」的 CFE 增益（系統級與逐客戶）。
...
（超出用電輪廓即外溢）。系統**帳面 RE% 63.4% → 逐時 CFE% 60.2%**，差距即時間錯配；
...
![逐時（24/7 CFE）匹配 · 含風光互補增益](docs/images/cfe-hourly.png)
