...
**89.4% → 96.8%（+7.4 pt）**，系統數字被沒裝電池、卻佔投組約 75% 用電的用電廠 1 稀釋掉了。24 小時
圖上新增的**斜線填充帶**就是電池放出的電，下方另有一條 **SOC 曲線**（日均水位，同一小時在整月裡
的平均值）標出電池的電量水位。電池的充電來源可以來自任何一座案場的外溢（跨合約），但自家合約
優先；跨合約充電目前屬於**情境模擬**，轉供結算單與 T-REC 尚未反映電池的充放。
