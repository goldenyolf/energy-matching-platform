...
- **狀態**：✅ 已實作（2026-07-29）— 對應 Roadmap `A7`（太陽能／技術別實體）、`B4`（風光互補混合最佳化），Phase 6
- **日期**：2026-07-29
...
## 11. 實作結果（2026-07-29）
**驗收數字**（示範資料 2024-01，interval 來源）：全系統 只風電 **58.79% → 風光 60.22%（+1.42 pt）**；
簽了光電的用電廠 2（電源管理）**73.32% → 89.39%（+16.07 pt）**。全套閘門綠（ruff／black／mypy／285 passed），
**零 migration**（沿用 `farm_type`）、**引擎零改動**。

與 §5／§6 的三處偏離（實作時的判斷）：

| 項目 | 規劃 | 實作 | 原因 |
|---|---|---|---|
| 回傳欄位 | `wind_only_cfe` + `combined_cfe` + `uplift` | `wind_only_cfe_percent` + `uplift_pt` + `solar_generation_by_hour`（combined 就是既有的 `cfe_percent`） | 不重複既有欄位；多回一條太陽能分量給前端畫圖 |
| 24h 圖 | 風＋光**堆疊區域** | 虛線＝只風電、實線＝風光合計，**中間琥珀色帶＝太陽能** | 堆疊區域被綠色匹配區蓋住幾乎看不見；改成疊在最上層的帶狀才讀得出來，也直接呼應 uplift 讀數 |
| 增益層級 | 僅系統級 | 系統級**＋逐客戶**（客戶表新增「風光增益」欄，選客戶時上方讀數跟著切） | 系統級被沒簽光電的大客戶稀釋（用電廠 1 佔約 74% 用電）：+1.42 pt 看不出效果，逐客戶的 +16.07 pt 才是證據。經使用者確認後加上 |

§7 副作用逐頁檢查結果：`farmTypeBadge` 已支援 solar（案場頁＋投資效益頁）、案場表單類型下拉加入「太陽能」、
投資效益頁在有 solar 時標示「沿用同一組每 MW CAPEX 假設，solar 的 ROI 僅供結構參考」（CAPEX 分技術別留待後續）。

**觀察**：示範投組把光電放大到 200 MW 以上，系統級增益就飽和在 +2.2 pt——瓶頸不是裝置容量，
而是光電只簽得到一家日間客戶，多的電只會外溢。這本身就是平台想講的事：**綠電的價值取決於配對，不只取決於裝多少**。
